...
tags:
  - Divide and conquer
lastModified: '2021-09-11T09:56:00.000Z'
teaser: 'Codeforces Round #702(Div.3) - problem D solution'
...
여기서 실제 이진 트리를 구성할 필요는 전혀 없습니다.  
아래는 _**divide and conquer**_에 기반한 풀이입니다.

$depth \leftarrow [0, 0, … 0]$  
헬퍼 함수 `findDepth(left, right, currentDepth)`에서,

1. $[left, right]$ 구간의 최댓값을 찾고, depth 값 `depth[mid]`를 `currentDepth`로 갱신합니다.
2. `findDepth(left, mid-1, currentDepth+1)`을 재귀적으로 호출합니다.
3. `findDepth(mid+1, right, currentDepth+1)`을 재귀적으로 호출합니다.

main 함수에서는 단순히 헬퍼 함수 `findDepth(0, n-1, 0)`을 호출하고 `depth` 배열을 출력합니다.

## Code

![](/blog/images/codeforces-1490D-result.png)
